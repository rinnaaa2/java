...
public class server extends javax.swing.JFrame { //наследование
    ...
    private static final int EXPECTED_CLIENTS = 9; // Ждем 9 клиентов
    private static final int THREADS_PER_CLIENT = 9; // Каждый клиент использует 9 потоков
    
    private Map<Integer, InetAddress> clients = new HashMap<>(); // пара номер клиента - адрес
 ...
 private void startServer() {
        try {
            ...
            new Thread(() -> { //бесконечно принимаем пакеты
                byte[] buffer = new byte[4096];
                while (true) {
                    try {
                        DatagramPacket packet = new DatagramPacket(buffer, buffer.length); //UDP пакет 
                        socket.receive(packet); //сокет ждет входящий пакет 
                        
                        ByteArrayInputStream bais = new ByteArrayInputStream(packet.getData(), 0, packet.getLength());
                        DataInputStream dis = new DataInputStream(bais);
                        
                        String command = dis.readUTF(); //читаем команду 
                        
                        if (command.equals("REGISTER")) { //клиент подключился  
                            int clientId = dis.readInt(); // клиент присылает свой ID
                            clients.put(clientId, packet.getAddress());
                            System.out.println("Клиент " + clientId + " подключился: " + packet.getAddress());
                            System.out.println("Подключено клиентов: " + clients.size() + " из " + EXPECTED_CLIENTS);
                            
                        } else if (command.equals("RESULT")) { //посчитали интеграл 
                            ...
                            if (taskData != null) {
                                taskData[0] += result; //промежутоный результат вычисления
                                taskData[1]++; // сколько клиентов вычислили 
                                
                                System.out.println("Получен результат от клиента " + clientId + 
                                    ": " + result + " (получено " + (int)taskData[1] + " из " + EXPECTED_CLIENTS + ")");
                                
                                // Если все 9 клиентов ответили
                                if (taskData[1] >= EXPECTED_CLIENTS) {
                                    final double finalResult = taskData[0];
                                    final int row = taskRows.get(taskId);
                                    
                                    SwingUtilities.invokeLater(() -> {
                                        tableModel.setValueAt(String.format("%.6f", finalResult), row, 3);
                                        Rasschet.setEnabled(true);
                                        updateCollection();
                                    });
                                    
                                    System.out.println("Задача " + taskId + " завершена! Итог: " + finalResult);
                                }
                            }
                        }
                    } catch (Exception e) {
            ...
            System.out.println("Ожидается подключение " + EXPECTED_CLIENTS + " клиентов");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
private RecIntegral findExistingRecord(double lowerLimit, double upperLimit, double step) {
    ...
    final double lowerLimit = Double.parseDouble(
        tableModel.getValueAt(selectedRow, 0).toString());
    final double upperLimit = Double.parseDouble(
        tableModel.getValueAt(selectedRow, 1).toString());
    final double step = Double.parseDouble(
        tableModel.getValueAt(selectedRow, 2).toString());
        
    Rasschet.setEnabled(false);
    
    String taskId = UUID.randomUUID().toString(); //генерируем id задачи 
    tasks.put(taskId, new double[]{0, 0});
    taskRows.put(taskId, selectedRow);
    
    double interval = (upperLimit - lowerLimit) / 9;
    // Отправляем задачи 9 клиентам
        for (int i = 0; i < 9; i++) {
            int clientId = i + 1;
            InetAddress client = clients.get(clientId);
            
            if (client != null) {
                double clientLower = lowerLimit + i * interval;
                double clientUpper = (i == 8) ? upperLimit : clientLower + interval;
                ...
                try {
                    ...
                    DatagramPacket packet = new DatagramPacket(data, data.length, client, 9877 + clientId);
                    socket.send(packet);
                    
                    System.out.println("Клиент " + clientId + ": [" + clientLower + ", " + clientUpper + "]");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        ...
        System.out.println("Задача " + taskId + " распределена между 9 клиентами");
