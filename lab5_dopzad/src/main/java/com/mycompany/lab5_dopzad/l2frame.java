 ...
 */
package com.mycompany.lab5_dopzad;
...
private RecIntegral findExistingRecord(double lowerLimit, double upperLimit, double step) {
    ...
    //private void Rasschet(java.awt.event.ActionEvent evt) {                          
    ...
    new Thread(() -> {
        java.util.concurrent.ExecutorService executor = java.util.concurrent.Executors.newFixedThreadPool(9);
        try {
...
//                threads[i].start();
                RecIntegral calculator = new RecIntegral(threadLower, threadUpper, step);
                java.util.concurrent.Future<Double> future = executor.submit(calculator);
                futures.add(future);
            }
            ...
            javax.swing.SwingUtilities.invokeLater(() -> {
                tableModel.setValueAt(String.format("%.6f", finaltotal), selectedRow, 3);
            ...
            updateCollection();
                ...
                System.out.println("Расчет выполнен в 9 потоках. Результат: " + finaltotal);
                System.out.println("Время выполнения: " + timeres);
            });
